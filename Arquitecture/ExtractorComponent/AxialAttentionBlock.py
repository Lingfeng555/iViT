...
if __name__ == "__main__":
    ...
    x = torch.randn(batch_size, num_features, 7, 7)

    attention_block = AttentionBlock(num_heads=num_features, attention_value=1)
    output = attention_block(x)
    print(output.shape) 
    print(f"Model parameters: {sum(p.numel() for p in attention_block.parameters())}")